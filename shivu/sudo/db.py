from shivu import sudo as sudo_collection  # from init, this is our MongoDB collection
...
async def get_appointed_by(user_id: int):
    data = await sudo_collection.find_one({"_id": user_id})
    return data["appointed_by"] if data else None


async def get_all_by_role(role: str):

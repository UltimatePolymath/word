...
async def find_ids_callback(client: Client, callback_query):
    ...
    try:
        ...
        response = await client.listen.Message(filters.text & ~filters.command, user_id=user_id, timeout=60)
        ...
        LOGGER.warning(f"User {user_id} timed out during name search")
...
async def search_id_callback(client: Client, callback_query):
    ...
    try:
        ...
        LOGGER.warning(f"User {user_id} timed out during ID search")
        await callback_query.message.reply_text("Search timed out. Please try again.")
    except Exception as e:
...
async def debug_command(client: Client, message):
    ...
    user_id = message.from_user.id
    chat_id = message.chat.id
    LOGGER.info(f"User {user_id} invoked /debug in chat {chat_id}")
